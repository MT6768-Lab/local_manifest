...
Several packages are needed to build CherishOS. You can install these using your distribution's package manager.
...
To build CherishOS, you'll need:
...
The `~/bin` directory will contain the git-repo tool (commonly named "repo") and the `~/android/crdroid` directory will contain the source code of CherishOS.
...
### Initialize the CherishOS source repository
...
The CherishOS manifests include a sensible default configuration for repo, which we strongly suggest you use (i.e. don't add any options to sync).
...
The `repo sync` command is used to update the latest source code from CherishOS and Google. Remember it, as you may want to
